...
const ROM = new Image();
const RAM = document.createElement("canvas");
...
function setNewScene(new_scene, RAM_SETUP, RAM_REQUEST) {
  ...
  switch(RAM_SETUP) {
    ...
    case "game":{
      RAM.width = 1216; RAM.height = 1216;
      transmit.drawImage(ROM,RAM_REQUEST[0].x,RAM_REQUEST[0].y,160,64,416,0,160,64);
      transmit.drawImage(ROM,RAM_REQUEST[1].x,RAM_REQUEST[1].y,416,128,0,0,416,128);
      transmit.drawImage(ROM,RAM_REQUEST[2].x,RAM_REQUEST[2].y,192,384,0,128,192,384);
      transmit.drawImage(ROM,RAM_REQUEST[3].x,RAM_REQUEST[3].y,192,384,192,128,192,384);
      transmit.drawImage(ROM,RAM_REQUEST[4].x,RAM_REQUEST[4].y,192,128,384,128,192,128);
      transmit.drawImage(ROM,RAM_REQUEST[5].x,RAM_REQUEST[5].y,128,64,384,256,128,64);
      transmit.drawImage(ROM,RAM_REQUEST[6][0].x,RAM_REQUEST[6][0].y,160,240,0,576,160,240);
      transmit.drawImage(ROM,RAM_REQUEST[6][1].x,RAM_REQUEST[6][1].y,160,240,160,576,160,240);
      transmit.drawImage(ROM,RAM_REQUEST[7][0].x,RAM_REQUEST[7][0].y,160,240,320,576,160,240);
      transmit.drawImage(ROM,RAM_REQUEST[7][1].x,RAM_REQUEST[7][1].y,160,240,480,576,160,240);
      transmit.drawImage(ROM,RAM_REQUEST[8].x,RAM_REQUEST[8].y,360,400,0,816,360,400);
      transmit.drawImage(ROM,RAM_REQUEST[9].x,RAM_REQUEST[9].y,360,400,360,816,360,400);
      transmit.drawImage(ROM,RAM_REQUEST[10].x,RAM_REQUEST[10].y,160,192,384,320,160,192);
      screen_fade = 255;
      game_scene = new_scene;
    };break;
  };
...
//setNewScene(9,"game",[ROM_DATA.game_score,ROM_DATA.skins[0],ROM_DATA.charaField[menu_char_select[0]][0],ROM_DATA.charaField[menu_char_select[1]][1]])
...
function debugMenu() {
  ...
  CTX.fillText("Lilyfuse   [PREVIEW2]",0,40);
...
class GameField {
  ...
  drawQueue() {
    ...
    for (let i = 0; i < 2; i++)
    this.queue_next_ctx.drawImage(
      RAM,
      (this.queue.next[i] - 1) * 32,
      0,
      32,
      32,
      i*32,
  ...
  drawScore(player_position) {
    ...
    for (let i = 0; i < 8; i++)
    this.score_ctx.drawImage(
      RAM,
      416+((n.charCodeAt(i)-48)*16),
      player_position * 32,
      16,
      32,
      i * 16,
  ...
  drawCursor() {
    ...
    this.queue_ctx.drawImage(
      RAM,
      (this.queue.current[0] - 1) * 32,
      0,
      32,
      32,
      (this.game_cursor.pos * 32) + (this.game_cursor.swap * 8),
      this.game_cursor.timer,
      32,
      32
    );
    this.queue_ctx.drawImage(
      RAM,
      (this.queue.current[1] - 1) * 32,
      0,
      32,
      32,
      ((this.game_cursor.pos + 1) * 32) - (this.game_cursor.swap * 8),
      this.game_cursor.timer,
      32,
      32
    );
  ...
  drawLoser(player_position) {
    CTX.drawImage(
      RAM,
      (player_position * 320) + 160,
      576,
      160,
      240,
      this.canvas_position + 16,
      192,
      160,
      240
    );
  ...
  drawWinLose(player_position) {
    CTX.drawImage(
      RAM,
      384,
      320 + (player_position == session.winner ? 0 : 96),
      160,
      96,
      this.canvas_position + 16,
      80,
    ...
    for (let y = 0; y < 12; y++) {
      for (let x = 0; x < 6; x++) {
        if (this.field_data[y][x].id != 0 && !this.field_data[y][x].clear) {
          this.field_ctx.drawImage(
            RAM,
            (this.field_data[y][x].id - 1) * 32,
            this.field_data[y][x].essence ? 64 : 0,
            32,
            32,
            x*32,
            (y*32) + this.field_shake,
            32,
            32
          );
        };
...
function game2P() {
  ...
  if (!session.start) {
    ...
    if (screen_fade > 0) screen_fade -= 16;
      ...
      else if (session.waitcount > 0) session.waitcount--;
      else {session.counttime = 480;session.waitcount = 4;session.countdown = 3;};
    }else if (session.countdown == 3) {
      CTX.drawImage(RAM,384,192,64,64,288-(session.counttime/2),192-(session.counttime/2),64+session.counttime,64+session.counttime);
      if (session.counttime > 0) session.counttime -= 32;
      else if (session.waitcount > 0) session.waitcount--;
      else {session.counttime = 480;session.waitcount = 4;session.countdown = 2;};
    }else if (session.countdown == 2) {
      CTX.drawImage(RAM,448,192,64,64,288-(session.counttime/2),192-(session.counttime/2),64+session.counttime,64+session.counttime);
      if (session.counttime > 0) session.counttime -= 32;
      else if (session.waitcount > 0) session.waitcount--;
      else {session.counttime = 480;session.waitcount = 4;session.countdown = 1;};
    }else if (session.countdown == 1) {
      CTX.drawImage(RAM,512,192,64,64,288-(session.counttime/2),192-(session.counttime/2),64+session.counttime,64+session.counttime);
      if (session.counttime > 0) session.counttime -= 32;
      else if (session.waitcount > 0) session.waitcount--;
      else {session.counttime = 480;session.waitcount = 4;session.countdown = 0;};
      ...
      case 0:/*Player Control*/{
        ...
        /*Process Player Input Functions*/{
          ...
          switch(inputArray[i]) {
            ...
            case controller[t]._b:/*Pause Session*/{}; break;
        ...
        /*Process Deleting/Adjusting Cleared Tiles*/{
          if (p[t].field_clear <= 0) {
            ...
            /*Count Number Of Tiles Fallen*/{
              ...
              if (fallCount <= 0) p[t].game_turnmode = 0;
              else {
                p[t].field_shake = 16;
                p[t].game_wait = 3;
                p[t].game_turnmode = 4;
